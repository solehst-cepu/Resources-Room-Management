        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <div className="relative">
            <button
              ...
              className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg bg-slate-100 hover:bg-slate-200/80 text-xs font-semibold text-slate-700 border border-slate-200 transition-colors cursor-pointer"
              title="Ganti Peran Pengguna untuk Simulasi"
            >
              <UserCheck className="w-3.5 h-3.5 text-blue-600" />
              <span className="hidden sm:inline text-slate-500">Peran:</span>
              <span className="font-bold text-blue-900">{currentUser?.name.split(' ')[0]}</span>
            ...
            {showRoleMenu && (
              ...
              >
                <div className="px-3 py-1.5 border-b border-slate-100">
                  <p className="text-[11px] font-bold text-slate-400 uppercase tracking-wider">Pilih Akun Demo Simulasi</p>
                </div>
                <div className="divide-y divide-slate-100 max-h-64 overflow-y-auto">
                  {users.map((u) => (
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 flex items-center justify-between hover:bg-blue-50/60 transition-colors cursor-pointer ${currentUser?.id === u.id ? 'bg-blue-50/80 font-bold' : ''}`}
                    >
                      <div className="flex items-center gap-2.5 min-w-0">
                        <img src={u.avatar} alt={u.name} className="w-7 h-7 rounded-full object-cover border border-slate-200 shrink-0" />
                        <div className="min-w-0">
                          <p className="text-xs text-slate-800 truncate">{u.name}</p>
                          <p className="text-[10px] text-slate-400 truncate">{u.unit} - {u.department}</p>
                        </div>
                      </div>
